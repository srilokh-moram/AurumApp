...
from services.mt5_service import (
    _get_tick, _close_position, _get_live_profits, _get_closed_profit, _check_pending_fills,
)
...
# ── Background: detect filled/cancelled pending orders ───────────────────────

def _sync_pending_orders_loop():
    while True:
        try:
            db: Session = SessionLocal()
            try:
                pending = db.query(PendingOrder).filter(PendingOrder.status == "pending").all()
                if pending:
                    tickets = [po.mt5_ticket for po in pending]
                    fills = _check_pending_fills(tickets)

                    for po in pending:
                        outcome = fills.get(po.mt5_ticket)
                        if not outcome:
                            continue

                        if outcome["status"] == "filled":
                            pos_ticket = outcome["position_ticket"]
                            ...
                            if pos_ticket:
                                ...
                                if not existing:
                                    db.add(Position(
                                        ...
                                        volume=float(po.volume),
                                        lot_size=float(po.lot_size),
                                        grid_gap=0,
                                    ...
                                    db.add(Transaction(
                                        ...
                                        lot_size=float(po.lot_size),
                                        mt5_ticket=pos_ticket,
                                        note=f"Pending {po.direction.upper()} filled @ {fill_price}",
                                    ))
...
def _launch_bg_threads():
    ...
    time.sleep(3)
    for fn in (_sync_floating_pl_loop, _sync_margin_call_loop, _sync_daily_snapshot_loop, _sync_pending_orders_loop):
