...
import { AdminUser, AdminPosition, AdminPositionDetail, AdminTransaction, AdminWithdrawalRequest } from "../types";
import api from "../api";
import { format } from "date-fns";
import { Users, DollarSign, BarChart3, TrendingUp, Settings, History, Layers, ArrowDownCircle, ArrowUpCircle, Clock } from "lucide-react";
...
export default function Admin() {
  ...
  function loadWithdrawals() {
    api.get("/admin/withdrawals?status=all").then((r) => setWdRequests(r.data));
  }
  ...
  function load() {
    Promise.all([
      ...
      api.get("/admin/withdrawals?status=all"),
    ]).then(([u, p, s, c, t, w]) => {
      setUsers(u.data);
      setPositions(p.data);
      setStats(s.data);
  ...
  async function approveWithdrawal(id: number) {
    if (!confirm("Approve this withdrawal? Funds will be deducted from the user's balance immediately.")) return;
    setApprovingId(id);
    try {
  ...
  // Per-user deposit/withdrawal summary derived from transaction history
  // Must be declared before any early return to satisfy Rules of Hooks
  const fundingByUser = useMemo(() => {
    const map: Record<number, { name: string; email: string; deposits: number; depCount: number; withdrawals: number; wdlCount: number }> = {};
    txs.forEach((t) => {
      if (t.type !== "deposit" && t.type !== "withdrawal") return;
      if (!map[t.user_id]) map[t.user_id] = { name: t.user_name, email: t.user_email, deposits: 0, depCount: 0, withdrawals: 0, wdlCount: 0 };
      if (t.type === "deposit") { map[t.user_id].deposits += t.amount; map[t.user_id].depCount++; }
      else { map[t.user_id].withdrawals += Math.abs(t.amount); map[t.user_id].wdlCount++; }
    });
    return Object.values(map).sort((a, b) => b.deposits - a.deposits);
  }, [txs]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <div className="card flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-emerald-400 font-mono">{fmt(stats.total_deposits)}</p>
              <p className="text-xs text-gray-600">{stats.deposit_count} transaction{stats.deposit_count !== 1 ? "s" : ""}</p>
            </div>
          </div>
          <div className="card flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-red-400 font-mono">{fmt(stats.total_withdrawals)}</p>
              <p className="text-xs text-gray-600">{stats.withdrawal_count} transaction{stats.withdrawal_count !== 1 ? "s" : ""}</p>
            </div>
          </div>
          <div className="card flex items-center gap-3">
            <div className="p-2 bg-gold-400/10 rounded-lg shrink-0">
            ...
            <div>
              <p className="text-xs text-gray-500">Net Funds</p>
              <p className={`text-xl font-bold font-mono ${(stats.total_deposits - stats.total_withdrawals) >= 0 ? "text-gold-400" : "text-red-400"}`}>
                {fmt(stats.total_deposits - stats.total_withdrawals)}
              </p>
            </div>
          </div>
          <div className="card flex items-center gap-3">
          ...
          <div className="card flex items-center gap-3">
            <div className={`p-2 rounded-lg shrink-0 ${totalFloating >= 0 ? "bg-emerald-500/10" : "bg-red-500/10"}`}>
              <TrendingUp size={16} className={totalFloating >= 0 ? "text-emerald-400" : "text-red-400"} />
            </div>
            <div>
      ...
      {/* Withdrawal Requests */}
      {(() => {
        const pending = wdRequests.filter((w) => w.status === "pending");
        const reviewed = wdRequests.filter((w) => w.status !== "pending");
        return (
          <div className="card">
            <div className="flex items-center gap-2 mb-5">
              ...
              {pending.length > 0 && (
                <span className="bg-amber-400/20 text-amber-400 text-xs px-2 py-0.5 rounded-full font-semibold">
            ...
            {wdRequests.length === 0 ? (
              <p className="text-gray-600 text-sm text-center py-8">No withdrawal requests yet</p>
            ) : (
              <div className="space-y-6">
                {/* Pending — shown first, highlighted */}
                {pending.length > 0 && (
                  <div className="space-y-3">
                    ...
                    <div className="md:hidden space-y-3">
                      {pending.map((w) => (
                        <div key={w.id} className="bg-amber-400/5 border border-amber-400/20 rounded-xl p-4 space-y-3">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <p className="font-medium text-white text-sm">{w.user_name}</p>
                              <p className="text-xs text-gray-500">{w.user_email}</p>
                            </div>
                            <p className="font-mono font-bold text-lg text-white shrink-0">{fmt(w.amount)}</p>
                          </div>
                    ...
                    <div className="hidden md:block overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-[#1f2937]">
                            {["User", "Amount", "Note", "Requested", "Actions"].map((h) => (
                              <th key={h} className="text-left py-3 px-3 text-xs text-gray-500 font-medium uppercase tracking-wider whitespace-nowrap">{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[#1f2937]">
                          {pending.map((w) => (
                            <tr key={w.id} className="bg-amber-400/5">
                              <td className="py-3 px-3">
                                <p className="font-medium text-white text-sm">{w.user_name}</p>
                                <p className="text-xs text-gray-500">{w.user_email}</p>
                              </td>
                              <td className="py-3 px-3 font-mono font-bold text-white">{fmt(w.amount)}</td>
                ...
                {reviewed.length > 0 && (
                  <div className="space-y-3">
                    <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">History</p>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-[#1f2937]">
                            {["User", "Amount", "Status", "Note", "Requested", "Reviewed"].map((h) => (
                              <th key={h} className="text-left py-3 px-3 text-xs text-gray-500 font-medium uppercase tracking-wider whitespace-nowrap">{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[#1f2937]">
                          {reviewed.map((w) => (
                            <tr key={w.id} className="hover:bg-[#1f2937]/30 transition-colors">
                              <td className="py-3 px-3">
                                <p className="font-medium text-white text-xs">{w.user_name}</p>
                                <p className="text-xs text-gray-500">{w.user_email}</p>
                              </td>
                              <td className="py-3 px-3 font-mono font-semibold text-white text-xs">{fmt(w.amount)}</td>
                              <td className="py-3 px-3">
                              ...
                              <td className="py-3 px-3 text-xs text-gray-400 max-w-[160px] truncate">{w.note || "—"}</td>
                              <td className="py-3 px-3 text-xs text-gray-500 whitespace-nowrap">{format(new Date(w.created_at), "MMM d, HH:mm")}</td>
                              <td className="py-3 px-3 text-xs text-gray-500 whitespace-nowrap">
                                {w.reviewed_at ? format(new Date(w.reviewed_at), "MMM d, HH:mm") : "—"}
                              </td>
                            </tr>
                          ))}
                        </tbody>
      ...
      <div className="card">
        <div className="flex items-center gap-2 mb-5">
          <ArrowDownCircle size={16} className="text-emerald-400" />
          <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Deposits & Withdrawals</h2>
          <span className="bg-gold-400/10 text-gold-400 text-xs px-2 py-0.5 rounded-full">{fundingByUser.length} users</span>
        </div>

        {fundingByUser.length === 0 ? (
          <p className="text-gray-600 text-sm text-center py-8">No deposits or withdrawals yet</p>
        ) : (
          <>
            {/* Mobile cards */}
            <div className="md:hidden space-y-3">
              {fundingByUser.map((u) => (
                <div key={u.email} className="bg-[#0f1117] rounded-xl border border-[#1f2937] p-4 space-y-2">
                  <div>
                  ...
                  <div className="flex justify-between pt-1">
                    <div>
                      <p className="text-[10px] text-emerald-500 uppercase tracking-wider mb-0.5">Deposited</p>
                      <p className="font-mono font-semibold text-emerald-400">{fmt(u.deposits)}</p>
                      <p className="text-[10px] text-gray-600">{u.depCount} time{u.depCount !== 1 ? "s" : ""}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] text-red-500 uppercase tracking-wider mb-0.5">Withdrawn</p>
                      <p className="font-mono font-semibold text-red-400">{u.withdrawals > 0 ? fmt(u.withdrawals) : "—"}</p>
                      <p className="text-[10px] text-gray-600">{u.wdlCount > 0 ? `${u.wdlCount} time${u.wdlCount !== 1 ? "s" : ""}` : ""}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-0.5">Net</p>
                      <p className={`font-mono font-semibold ${(u.deposits - u.withdrawals) >= 0 ? "text-gold-400" : "text-red-400"}`}>
                        {fmt(u.deposits - u.withdrawals)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#1f2937]">
                    {["User", "Total Deposited", "Deposit Count", "Total Withdrawn", "Withdrawal Count", "Net"].map((h) => (
                      <th key={h} className="text-left py-3 px-3 text-xs text-gray-500 font-medium uppercase tracking-wider whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#1f2937]">
                  {fundingByUser.map((u) => (
                    <tr key={u.email} className="hover:bg-[#1f2937]/30 transition-colors">
                      <td className="py-3 px-3">
                        <p className="font-medium text-white text-sm">{u.name}</p>
                        <p className="text-xs text-gray-500">{u.email}</p>
                      </td>
                      <td className="py-3 px-3 font-mono font-semibold text-emerald-400">{fmt(u.deposits)}</td>
                      <td className="py-3 px-3 text-gray-300">
                        <span className="bg-emerald-500/10 text-emerald-400 text-xs px-2 py-0.5 rounded-full font-medium">
                          {u.depCount} deposit{u.depCount !== 1 ? "s" : ""}
                        </span>
                      </td>
                      <td className="py-3 px-3 font-mono font-semibold text-red-400">
                        {u.withdrawals > 0 ? fmt(u.withdrawals) : <span className="text-gray-600">—</span>}
                      </td>
                      <td className="py-3 px-3 text-gray-300">
                        {u.wdlCount > 0 ? (
                          <span className="bg-red-500/10 text-red-400 text-xs px-2 py-0.5 rounded-full font-medium">
                            {u.wdlCount} withdrawal{u.wdlCount !== 1 ? "s" : ""}
                          </span>
                        ) : <span className="text-gray-600 text-xs">—</span>}
                      </td>
                      <td className={`py-3 px-3 font-mono font-semibold ${(u.deposits - u.withdrawals) >= 0 ? "text-gold-400" : "text-red-400"}`}>
                        {fmt(u.deposits - u.withdrawals)}
                      </td>
                    </tr>
                  ))}
